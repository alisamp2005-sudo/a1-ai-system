...
echo -e "${YELLOW}[3/5]${NC} Запуск Docker Compose (7 контейнеров)..."
docker compose up -d --build
echo -e "  ${GREEN}✅ Docker Compose запущен${NC}"
echo ""

# 4. Запустить Vision-сервис (в фоне)
echo -e "${YELLOW}[4/5]${NC} Запуск Vision-сервиса..."
...
# 5. Запустить Cloudflare Tunnel (в фоне)
echo -e "${YELLOW}[5/5]${NC} Запуск Cloudflare Tunnel..."
