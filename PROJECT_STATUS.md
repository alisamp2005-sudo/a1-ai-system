...
| RAG (база знаний) | ⚠️ Частично | ChromaDB выдаёт ошибку `tenant default_tenant` из Docker. Нужно пересоздать коллекцию |
...
| Синхронизация Яндекс.Диска | ⚠️ Частично | Ошибка `tuple` при обработке Excel. Папки сканируются, но файлы внутри не скачаны |
...
1. **ChromaDB `tenant default_tenant` из Docker** — RAG-поиск не работает из контейнеров. Документы загружены (70 фрагментов), но бот не может их найти. Нужно: проверить версию chromadb-клиента в Docker vs сервер, возможно пересоздать коллекцию или обновить клиент.
...
3. **Синхронизация Яндекс.Диска** — ошибка при обработке Excel (`tuple` object). Папки видит, но файлы внутри папок не скачивает (404 при запросе содержимого). Нужно: использовать `item.get('path')` из API вместо конструирования пути.
...
1. **Починить ChromaDB из Docker** — проверить совместимость версий клиента/сервера, пересоздать коллекцию `a1_knowledge_base` если нужно.
2. **Починить синхронизацию Яндекс.Диска** — исправить обход вложенных папок (использовать path из API), обработку Excel.
