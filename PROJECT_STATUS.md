...
- Персонализация для генерального директора: `/start` отвечает «Привет, Босс!», а содержательные ответы получают краткое деловое обращение «Босс,»; для остальных сотрудников стиль не меняется
...
- Inline-кнопки под ответами бота (объекты, сотрудники) → карточки в Mini App; для объектов используются канонические URL с кодированием и точным именем, поэтому разные кнопки не открывают одну и ту же карточку
...
- Оригиналы новых документов сохраняются локально в `/app/data/documents`; бот выдаёт их через Telegram только ролям `admin` и `top_manager` после проверки фактического Telegram ID
- Публичные URL для оригиналов не используются: это исключает несанкционированное скачивание по известной ссылке
...
- Синхронизация Яндекс.Диска автоматически backfill-ит файлы, которые были ранее проиндексированы только как текст, но не имеют сохранённого оригинала; это делает их доступными для выдачи через Telegram
- Celery worker явно импортирует модули SLA, дайджеста и синхронизации Яндекс.Диска. Это устраняет live-ошибку `Received unregistered task` для SLA-проверки
